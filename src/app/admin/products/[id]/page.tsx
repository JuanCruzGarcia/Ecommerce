    ...
    useEffect(() => {
        ...
        async function loadProduct() {
            ...
            const { data: product, error } = await supabase
                .from('products')
                .select('name, description, price, stock, category_id, active, is_featured, image_url, variants')
                .eq('id', id)
                .single();
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-5">
                            ...
                            <div
                                className={`flex items-center justify-between p-4 rounded-lg border mt-2 ${isFeatured ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200'} transition-all cursor-pointer`}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className={`font-semibold ${isFeatured ? 'text-yellow-900' : 'text-gray-900'}`}>{isFeatured ? 'Destacado' : 'No Destacado'}</span>
                                </div>
                                <div className={`w-10 h-6 flex items-center rounded-full p-1 transition-colors ${isFeatured ? 'bg-yellow-500' : 'bg-gray-300'}`}>
                                    <div className={`bg-white w-4 h-4 rounded-full shadow-sm transform transition-transform ${isFeatured ? 'translate-x-4' : 'translate-x-0'}`} />
                                </div>
                            </div>
                        </div>
