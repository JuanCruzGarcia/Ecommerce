    ...
    return (
        ...
        >
            <Link href={`/product/${product.id}`} className="block">
                <div className="relative aspect-[4/5] rounded-xl overflow-hidden mb-4 bg-slate-100 dark:bg-slate-800">
                    {/* Badge Categoría */}
                    {product.categories?.[0]?.name && (
                        <span className="absolute top-4 left-4 z-20 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md text-black dark:text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-sm">
                            {product.categories[0].name}
                        </span>
                    )}
                    ...
                    {/* Wishlist Button */}
                    <button className="absolute top-4 right-4 z-20 size-10 bg-white/10 dark:bg-black/10 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-primary transition-colors">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {product.stock <= 0 ? (
                            <span className="text-[10px] font-bold text-red-500 bg-red-50 dark:bg-red-500/10 px-2 py-1 rounded-full uppercase">Sin Stock</span>
                        ) : (
                            <p className="text-slate-500 dark:text-slate-400 text-xs font-medium">6 cuotas sin interés</p>
                        )}
                    </div>
                </div>
            </Link>
